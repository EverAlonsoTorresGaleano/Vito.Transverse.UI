...
const ApplicationCreate: React.FC = () => {
  ...
  useEffect(() => {
    const loadDropdowns = async () => {
      try {
        ...
        const [usersData, licenseTypesData] = await Promise.all([
  ...
  const onSubmit = async (data: ApplicationDTO) => {
    ...
    try {
      ...
      await client.postApiApplicationsV1(data);
  ...
  return (
    <Box>
      ...
      <Box sx={{ mb: 3, mt: 3 }}>
        ...
        >
          {t('ApplicationCreate_Title')}
        </Typography>
        <Typography variant="body1" sx={{ color: '#666666' }}>
          {t('ApplicationCreate_Subtitle')}
        </Typography>
      </Box>
